...
export function LoadingState({ text }: { text: string }) {
  return (
    <div className="rounded-xl bg-slate-50 px-5 py-10 text-center text-sm text-slate-500">
      <span className="mr-2 inline-block h-3.5 w-3.5 animate-spin rounded-full border-2 border-slate-300 border-t-slate-700 align-[-2px]" />
      {text}
    </div>
  );
}
...
export function Field({
  ...
  return (
    <label className="block">
      ...
      {children}
      {error ? (
        <span className="mt-1.5 block text-sm text-rose-700" role="alert">
          {error}
        </span>
      ) : null}
...
export function BackLink({ href, label }: { href: string; label: string }) {
  return (
    <Link href={href} className="inline-flex items-center gap-1 text-sm font-semibold text-slate-500 hover:text-slate-800">
      ← {label}
    </Link>
  );
}
